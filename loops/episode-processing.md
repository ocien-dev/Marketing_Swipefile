...
- After each lot, run `scripts/audit_insights_v2_text.py` to scan internal editorial fields (`canonical_title`, `specific_takeaway`, `use_case`, `when_to_use`, `when_not_to_use`) for non-ASCII characters, orphan `?` encoding artifacts, and normalized duplicate `specific_takeaway` values in the final v2 base. This non-ASCII rule applies to internal editorial fields only, not to evidence quotes or final human-facing outputs.
